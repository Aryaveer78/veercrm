...
The public API lets you drive your VEERCRM instance from your own
...
```
Authorization: Bearer VEERCRM_live_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
```
...
3. Copy the key. **The full key is shown exactly once.** VEERCRM
...
curl https://your-crm.example.com/api/v1/me \
  -H "Authorization: Bearer VEERCRM_live_xxx"
```
...
Rather than polling, register an endpoint and VEERCRM will POST to it when
...
- `POST /api/v1/webhooks` — register `{ "url": "https://…", "events": ["message.received"] }`. `url` must be `https://`. **The response includes `secret` exactly once** — store it to verify signatures; VEERCRM keeps only an encrypted copy.
...
curl -X POST https://your-crm.example.com/api/v1/webhooks \
  -H "Authorization: Bearer VEERCRM_live_xxx" \
  -H "Content-Type: application/json" \
  -d '{ "url": "https://example.com/hooks/VEERCRM", "events": ["message.received"] }'
...
Headers: `X-VEERCRM-Event`, `X-VEERCRM-Webhook-Id`, and `X-VEERCRM-Signature`.

### Verifying the signature

`X-VEERCRM-Signature: t=<unix_seconds>,v1=<hex>` where `v1 =
...
covers messages VEERCRM stores (inbox + API sends), not broadcast-only
...
[#245](https://github.com/ArnasDon/VEERCRM/issues/245). Future ideas
