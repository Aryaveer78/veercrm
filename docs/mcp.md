...
VEERCRM ships a [Model Context Protocol](https://modelcontextprotocol.io)
...
[`VEERCRM-mcp`](https://www.npmjs.com/package/VEERCRM-mcp). Under the hood
...
data or send messages, add `"VEERCRM_ENABLE_WRITES": "true"` (and
`"VEERCRM_ENABLE_BROADCASTS": "true"` for mass sends) to `env`.
