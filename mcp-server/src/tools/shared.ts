...
// layer dumb). On a VEERCRMApiError we return an `isError` result with
...
import { VEERCRMApiError } from '../client.js';
...
/**
 * Wrap a tool handler so any VEERCRMApiError becomes a clean, model-
...
export function handle<A>(
  ...
  return async (args: A) => {
    try {
      ...
      if (err instanceof VEERCRMApiError) {
        return errorResult(`VEERCRM API error [${err.code}]: ${err.message}`);
      }
