// ============================================================
// VEERCRM public API client.
//
// A thin wrapper over the `/api/v1` REST surface. It attaches the
// bearer key, unwraps the `{ data }` / `{ error }` envelope, and
// turns API failures into a typed VEERCRMApiError the tools can render
// cleanly. Nothing here knows about MCP — it's just the CRM API.
// ============================================================

import type { Config } from './config.js';

/** A structured error from the VEERCRM API envelope (`{ error: { code, message } }`). */
export class VEERCRMApiError extends Error {
  readonly status: number;
  readonly code: string;

  constructor(status: number, code: string, message: string) {
    super(message);
    this.name = 'VEERCRMApiError';
    this.status = status;
    this.code = code;
  }
}
...
export class VEERCRMClient {
  ...
  private async request<T>(
    ...
    try {
      ...
      throw new VEERCRMApiError(
        0,
        'network_error',
        `Could not reach VEERCRM at ${this.baseUrl}: ${(err as Error).message}`,
      );
    }
    ...
    if (text) {
      try {
        ...
        if (!res.ok) {
          throw new VEERCRMApiError(res.status, 'internal', text.slice(0, 500));
        }
      }
    }
    ...
    if (!res.ok) {
      ...
      throw new VEERCRMApiError(res.status, code, message);
    }
