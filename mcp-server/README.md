...
**[VEERCRM](https://github.com/ArnasDon/VEERCRM)** — the self-hostable
...
It's a thin wrapper over VEERCRM's public [`/api/v1`](../docs/public-api.md)
REST API. All auth, scoping, and rate limiting are enforced by your
VEERCRM instance — this server just exposes the API as MCP tools.

## Prerequisites

1. A running VEERCRM instance (your own self-hosted deploy).
...
| ------------------------- | -------- | -------------------------------------------------------------- |
| `VEERCRM_BASE_URL`          | yes      | Your instance URL, e.g. `https://crm.example.com`              |
| `VEERCRM_API_KEY`           | yes      | An API key from the dashboard                                  |
| `VEERCRM_ENABLE_WRITES`     | no       | `true` to expose contact writes + message sending             |
| `VEERCRM_ENABLE_BROADCASTS` | no       | `true` to expose mass broadcasts (needs `VEERCRM_ENABLE_WRITES`) |
...
```jsonc
{
  "mcpServers": {
    "VEERCRM": {
      ...
      "args": ["-y", "VEERCRM-mcp"],
      "env": {
        "VEERCRM_BASE_URL": "https://crm.example.com",
        "VEERCRM_API_KEY": "VEERCRM_live_xxxxxxxxxxxxxxxxxxxxxxxx"
      }
    }
  }
}
...
"env": {
  "VEERCRM_BASE_URL": "https://crm.example.com",
  "VEERCRM_API_KEY": "VEERCRM_live_xxxxxxxxxxxxxxxxxxxxxxxx",
  "VEERCRM_ENABLE_WRITES": "true",
  "VEERCRM_ENABLE_BROADCASTS": "true"
}
```
...
1. **Read-only by default.** Write and broadcast tools are not even
   ...
   `VEERCRM_ENABLE_WRITES` / `VEERCRM_ENABLE_BROADCASTS`.
2. **API-key scopes.** Whatever the guards allow, your VEERCRM instance
