...
describe('generateReply — Gemini', () => {
  it('calls the GenerateContent endpoint and maps Gemini usage', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      okResponse({
        candidates: [
          {
    ...
    const res = await generateReply({
      config: config({ provider: 'gemini', model: 'gemini-3.6-flash', apiKey: 'AIza-test' }),
      systemPrompt: 'sys',
      messages: [{ role: 'user', content: 'Hello' }],
    })
    ...
    expect(res).toEqual({
      ...
      handoff: false,
      usage: { promptTokens: 25, completionTokens: 7, totalTokens: 32 },
    })
    const [url, opts] = fetchMock.mock.calls[0]
  ...
  it('maps Gemini auth failures to invalid_key', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(
    ...
    await expect(
      generateReply({
        config: config({ provider: 'gemini', apiKey: 'bad-key' }),
        systemPrompt: 'sys',
        messages: [{ role: 'user', content: 'Hi' }],
      }),
    ).rejects.toMatchObject({ code: 'invalid_key', status: 401 })
  ...
  it('drops a leading assistant turn for Gemini', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(
        okResponse({
