...
export const RATE_LIMITS = {
  ...
  /** AI auto-reply generation, per account. Bounds the whole account
   *  across threads, so a burst of inbound from many customers at once
   *  can't run the BYO key past the provider's limit or the owner's
   *  budget. 30/min is generous for organic inbound while capping a
   *  stampede; excess inbounds simply don't get an auto-reply (they
   *  still land in the inbox for a human). */
