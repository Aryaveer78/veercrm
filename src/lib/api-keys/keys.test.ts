...
describe('hashApiKey', () => {
  it('is deterministic', () => {
    expect(hashApiKey('VEERCRM_live_abc')).toBe(hashApiKey('VEERCRM_live_abc'));
  });

  it('differs for different inputs', () => {
    expect(hashApiKey('VEERCRM_live_abc')).not.toBe(hashApiKey('VEERCRM_live_abd'));
  });
});
...
describe('timingSafeHexEqual', () => {
  it('is true for identical digests', () => {
    const h = hashApiKey('VEERCRM_live_xyz');
